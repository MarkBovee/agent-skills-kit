...
function Invoke-BootstrapManagedCheckoutPull {
    ...
    foreach ($statusLine in $statusLines) {
        ...
        if (-not $path.StartsWith(".claude/") -and -not $path.StartsWith(".dsh/") -and -not $path.StartsWith(".github/skills/") -and -not $path.StartsWith(".github/prompts/") -and $path -ne ".github/copilot-instructions.md") {
    ...
    & $git.Source -C $RepoRoot restore --source=HEAD --staged --worktree -- .claude .dsh .github/skills .github/prompts .github/copilot-instructions.md
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to restore generated platform artifacts in managed checkout $RepoRoot."
    }

    & $git.Source -C $RepoRoot clean -fd -- .claude .dsh .github/skills .github/prompts .github/copilot-instructions.md 1>$null 2>$null
    if ($LASTEXITCODE -ne 0) {
