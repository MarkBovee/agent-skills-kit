...
## Flow
**Capture** — record need, context, goals, scope, non-goals, stakeholders, constraints, risks, dependencies, compatibility boundaries, open questions, and assumptions. Flag ambiguity; never assume. Keep the artifact proportional: ordinary work may need only a short requirements and acceptance record. When a single requirement spans several independently testable capabilities, first map the capabilities and their dependency direction so the spec decomposes rather than staying a monolithic block.

**Structure** — produce one contract containing requirements, decisions, constraints, must-remain-unchanged behavior, non-goals, invariants, acceptance criteria, proof obligations, dependencies, compatibility, and change impact. Keep requirement, constraint, non-goal, protected behavior, and invariant distinct. Decisions record choice, owner, rationale, alternatives considered and their consequences, and status: `proposed`, `confirmed`, `rejected`, or `superseded`; only `confirmed` decisions are settled implementation input. AI may recommend options, but may not settle product, ownership, compatibility, or architectural decisions for a human owner. For each material or critical requirement, preserve where relevant:
