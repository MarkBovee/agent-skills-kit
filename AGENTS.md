...
- `node -e "import('./plugins/agent-skills-router/server.mjs').then(async m=>{const p=await m.AgentSkillsRouter(); await p.event({event:{type:'session.created',properties:{info:{id:'s'}}}}); const r=await p['tui.prompt.append']({sessionID:'s',prompt:'test'}); console.log(r?.append?.slice(0,200))})"` — test plugin hooks
  ...
  - Sidebar state uses the supported V2 `session.prompt` hook metadata field and the TUI's reactive `api.data.session` store. V2 has no `session.update`; plugin storage is not session metadata. Tool-hook changes become visible on next prompt because V2 exposes no server-to-TUI session-metadata mutation API.
  - The server plugin lives in OpenCode's automatically discovered local plugin directory; installers must not also register `./plugins/agent-skills-router` in `opencode.json`, or every prompt is injected twice. The TUI entry remains explicitly listed in `tui.json` because TUI plugins are not auto-discovered.
...
8. OpenCode plugin check: in a test session, verify `╌ Agent Skills Kit ╌` appears in the system prompt and status panel appears in sidebar. The server package must be present under the automatically discovered `plugins/agent-skills-router/` directory; it must **not** also appear in `opencode.json.plugins`. `tui.json` must list `./plugins/agent-skills-router/tui.tsx`, since TUI plugins are not auto-discovered. The package needs `server.mjs` plus `tui.tsx`.
