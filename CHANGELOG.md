...
- **First-class `research` and `deep-research` workflows.** ASK now distinguishes bounded evidence-first fact-finding from autonomous multi-source technical investigation. Deep research creates a research brief and dynamic tracks across local code, fixtures, logs, history, issues, upstream source, standards, documentation, and external evidence; it records hypotheses, contradictory evidence, confidence, citations, continuation state, and an implementation/debugging/specification handoff without starting implementation itself. Both skills route through OpenCode and dsh, ship slash commands and generated platform exports, and are verified through deterministic workflow and isolated-install checks.
- **Large-brief lifecycle and cross-platform workflow mandate.** `intake` now records an aggressiveness contract (`must`/`should`/`could`, deferral reason, revisit trigger) for maximum-result work, `agent-workflows` supplies a parallel research fan-out, and `session-review` has a host-neutral state fallback. Large multi-issue work routes into intake before execution. `rules/workflow.md` is installed into OpenCode and Claude and mirrored into Copilot and dsh; OpenCode receives an idempotent managed `AGENTS.md` section. Bash and PowerShell installer paths are both exercised in the isolated install check. (#60, #61, #62, #63, #64)
- **OpenCode ASK sidebar now de-duplicates active skills and normalizes its installed TUI entry.** Repeated state records can no longer render the same skill multiple times, pending items no longer show prompt-only `skill(name: '...')` actions, and both installers reduce `tui.json` to exactly one router sidebar entry on every refresh.
- **Renamed the `ui-ux` skill to `design`.** Interface work now loads `skill(name: 'design')` and uses `/design`; routing, generated platform exports, and installer migration remove the retired `ui-ux` identifiers. Loading `design` continues to require `design-review` before handoff.
- **ASK sidebar no longer lists unloaded route matches as active skills.** A route match such as `code-review` could appear alongside a loaded `Spec` skill, despite no code-review skill load. `ACTIVE SKILLS` now contains only actual skill loads, preserving one current skill and newest-first loaded context across OpenCode and dsh. Stateful OpenCode hooks now serialize per session, preventing an overlapping route update from restoring an older skill order.
- **ASK sidebar active-skill list no longer collapses to a single fallback.** The panel showed only `Develop` because the cascade's develop fallback overwrote the loaded skill and was itself rendered as active. A loaded skill now outranks a fresh route match, the fallback never appears as an active entry, and a non-matching prompt no longer resets the current skill.
- **A routed-but-unloaded skill no longer looks active next to its own pending obligation.** The sidebar marked any route match as `●` active, so a code-review prompt showed `Code Review` as active while `PENDING` simultaneously asked to load it — the same skill twice, and no honest active skill. Only a skill the agent actually loaded is marked `current`; a route match stays a hollow `○` suggestion until its skill loads.
- **ASK sidebar shows active skills instead of a workflow route, with a consistent two-section layout.** The canonical snapshot exposes `activeSkills` — the loaded skill first, then any matched or previously loaded skill — and the pending review obligations (`code-review`, `design-review`) with their concrete `skill(name: '<skill>')` actions. Route matches are hollow suggestions; only the loaded skill carries `current: true`, so the develop fallback can never displace it. `ACTIVE SKILLS` and `PENDING` share one header style and row grammar, and each pending action names the exact tool call instead of a vague `Load <skill>` label. Improvement capture is steered through the prompt surface, not the panel. The workflow route stays internal to the prompt surface. Removed the dead `workflowRoute` presenter, the stale `activeSkill`/`activeSkillLabel` state shape, and the redundant per-save `routing` snapshot (the panel is rebuilt from full state on every persist).
