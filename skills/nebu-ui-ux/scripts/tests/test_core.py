#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Stdlib-only regression tests for core.py / design_system.py.
...
class TestSearchDomains(unittest.TestCase):
    ...
    def test_zero_result_query_reports_suggestions(self):
...
class TestPersistence(unittest.TestCase):
    def test_persist_then_skip_then_force(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(master.read_text(encoding="utf-8"), original)
...
class TestReasoningMatch(unittest.TestCase):
    def test_known_category_matches_exactly(self):
        ...
        self.assertTrue(rule, "exact-match category lookup should not fall through to fuzzy")
